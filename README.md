...
Turn Claude (or any MCP-compatible AI) into your personal running coach. 

While most Strava integrations just fetch a basic list of your recent workouts, this server is built from the ground up for **Deep Analytics**. Powered by Python and `pandas`, it gives your AI agent the ability to analyze your heart rate zones, track your shoe mileage, detect your macro-training phases (Build vs. Taper), and even identify mid-run fatigue.

## ✘ Why use this one?

- **Python & Pandas Native:** Written in Python using `fastmcp`. It's incredibly easy to fork and extend if you want to pipe your running data into custom data science models or generate `matplotlib` graphs.
...
- `get_daily_briefing` - Ideal for ambient agents: a quick summary of yesterday's workouts and current weekly progress.
...
- `get_athlete_stats` - All-time and recent running totals.
